...
| **2. Database & Schema** | `REAL SCHEMA / HYBRID RUNTIME` | Production SQL migration [`20260828000000_production_schema.sql`](file:///d:/Personal%20projects/aura/supabase/migrations/20260828000000_production_schema.sql) with 12 core tables, indexes, and full RLS policies. | Apply migration SQL to target Supabase instance. |
| **3. Row Level Security (RLS)** | `REAL SQL POLICIES & TESTS VERIFIED` | RLS policies defined on all 12 tables + validated in [`securityRls.test.ts`](file:///d:/Personal%20projects/aura/__tests__/securityRls.test.ts). | Verified via test suite; apply to remote PostgreSQL. |
| **4. Image Storage** | `HYBRID & STORAGE SERVICE WIRED` | [`cloudStorageService.ts`](file:///d:/Personal%20projects/aura/src/services/storage/cloudStorageService.ts) implements authenticated bucket uploads, signed URLs, and account purge. | Provision buckets in Supabase Storage dashboard. |
| **5. Garment Image Processing** | `FALLBACK / SELF-HOSTED PENDING` | [`imageProcessingProvider.ts`](file:///d:/Personal%20projects/aura/src/services/image-processing/imageProcessingProvider.ts) extracts attributes deterministically with clean latency emulation. | Requires deploying self-hosted `aura-segment-v1` container (`BiRefNet` Apache 2.0). |
| **6. AI Stylist Engine** | `REAL (DETERMINISTIC)` | [`stylingEngine.ts`](file:///d:/Personal%20projects/aura/src/services/stylist/stylingEngine.ts) executes real combinatorial generation, weather filtering, 48hr recency damping, and preference weight learning. | Production ready for client-side execution; 100% owned TypeScript algorithms. |
| **7. Inspiration AI & Matching** | `REAL (MATCHING) / FALLBACK (VISION)` | [`inspirationMatchingService.ts`](file:///d:/Personal%20projects/aura/src/services/inspiration/inspirationMatchingService.ts) matches closet pieces into genuine quality tiers (`exact`, `close`, `similar`, `missing`). | Needs deploying self-hosted `aura-inspire-v1` container (`Florence-2-Large` MIT). |
| **8. Trend Intelligence** | `DEVELOPMENT / SEED` | [`trendIntelligenceService.ts`](file:///d:/Personal%20projects/aura/src/services/intelligence/trendIntelligenceService.ts) compares curated trend snapshots against owned clothes. | Needs live open fashion feed connector. |
| **9. Virtual Try-On (VTO)** | `ARCHITECTURE VERIFIED` | [`vtoProvider.ts`](file:///d:/Personal%20projects/aura/src/services/vto/vtoProvider.ts) implements state machine (`preparing` → `processing` → `generating` → `completed`). Zero secrets in mobile bundle. | Requires deploying self-hosted `aura-vto-v1` GPU container (`IDM-VTON` / `CatVTON`). |
