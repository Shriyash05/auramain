/**
 * Personal AURA Model Onboarding Flow
 * 
 * 5-Step Guided Onboarding:
 * 1. Proportions (Height, Weight, optional measurements)
 * 2. Usual Sizes (Tops, Bottoms, Shoes)
 * 3. Body Shape (Straight, Athletic, Broader Shoulders, Fuller Midsection, Curved)
 * 4. Face References (Identity appearance and facial proportions - strictly distinct from body)
 * 5. Model Ready (Private local storage, persistent model ready for Studio & Try-On)
 */
...
interface PersonalModelOnboardingModalProps {
  visible: boolean;
  userId: string;
  onClose: () => void;
  onCompleted: (model: AuraUserModel) => void;
}
...
export const PersonalModelOnboardingModal: React.FC<PersonalModelOnboardingModalProps> = ({
  visible,
  userId,
  onClose,
  onCompleted,
  ...
  useEffect(() => {
    if (visible && userId) {
      MirrorService.getUserModel(userId).then((model) => {
        if (model) {
  ...
  const handleSaveAndComplete = async () => {
    ...
    try {
      ...
      const savedModel = await MirrorService.saveUserModel(userId, {
        proportions,
        sizes,
      ...
      onCompleted(savedModel);
      onClose();
    } catch (e: any) {
      Alert.alert('Save Error', e.message || 'Could not save personal AURA model.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose}>
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.header}>
          <TouchableOpacity
            activeOpacity={0.7}
            onPress={step > 1 ? () => setStep((s) => (s - 1) as OnboardingStep) : onClose}
            style={styles.backBtn}
          >
            {step > 1 ? <ArrowLeft size={20} color={colors.text} /> : <X size={20} color={colors.text} />}
          </TouchableOpacity>
